...
namespace :xero do
  ...
  task status: :environment do
    ...
    rows = [
      ...
      [ "invoices",          "#{org.invoices.count}  #{range.(org.invoices, :issued_on)}" ],
      [ "bills",             "#{org.expenses.count}  #{range.(org.expenses, :incurred_on)}" ],
      [ "payments",          Payment.where(organization: org).count ],
      [ "journal entries",   "#{org.journal_entries.count}  #{range.(org.journal_entries, :posted_on)}" ],
