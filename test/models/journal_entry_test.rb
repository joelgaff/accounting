...
class JournalEntryTest < ActiveSupport::TestCase
  ...
  test "balanced 3-line journal posts to ledger" do
    doc = @org.documents.create!(date: Date.current, documentable: JournalEntry.new(
      narrative: "Petty cash reimb",
      lines_attributes: [
    ...
    assert je.balanced?
    assert_equal BigDecimal("150"), doc.total
  ...
  test "unbalanced entry is invalid, does not post" do
    doc = @org.documents.build(date: Date.current, documentable: JournalEntry.new(
      narrative: "Bad",
      lines_attributes: [
        { account_id: @office.id, debit_amount: 100 },
    ...
    assert_match(/debits.*must equal credits/, doc.errors.full_messages.join)
  end

  test "single-line entry is invalid" do
    doc = @org.documents.build(date: Date.current, documentable: JournalEntry.new(
      narrative: "Only one line",
      lines_attributes: [ { account_id: @office.id, debit_amount: 100 } ]))
    assert_not doc.valid?
    assert_includes doc.errors.full_messages.join, "at least two lines"
  end
